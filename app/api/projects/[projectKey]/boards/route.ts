...
async function fetchAllPaginated(
  ...
  while (hasMore) {
    try {
      ...
      console.log(
        `Fetched ${data.values.length} boards. Total so far: ${allResults.length}`
      )

      // Safety check to prevent infinite loops
      if (allResults.length > 1000) {
        console.warn(
          'Reached safety limit of 1,000 boards. Stopping pagination.'
        )
        break
      }
    } catch (error) {
...
export async function GET(
  ...
  try {
    ...
    console.log(
      `Found ALL ${boards.length} boards for project ${params.projectKey}`
    )
