  ...
  while (hasMore) {
    try {
      ...
      if (allResults.length > 10000) {
        console.warn("Reached safety limit of 10,000 items. Stopping pagination.")
        break
      }
    } catch (error) {
...
export async function getProjectSprints(
  ...
  try {
    ...
    const allBoards = await fetchAllPaginated(async (startAt, maxResults) => {
      return await jiraAgileFetch(`/board?projectKeyOrId=${projectKey}&startAt=${startAt}&maxResults=${maxResults}`)
    }, 50)

    console.log(`Found ${allBoards.length} total boards for project ${projectKey}`)
    ...
    console.log(`Using board: ${boardToUse.name} (ID: ${boardToUse.id}, Type: ${boardToUse.type})`)
    ...
    const allSprints = await fetchAllPaginated(async (startAt, maxResults) => {
      return await jiraAgileFetch(`/board/${boardToUse.id}/sprint?startAt=${startAt}&maxResults=${maxResults}`)
    }, 50)
    ...
    console.log(`Successfully fetched ALL ${sprints.length} sprints for project ${projectKey}`)
    ...
    try {
      console.log("Attempting fallback JQL-based sprint search for ALL sprints...")
      ...
      if (allIssues.length > 0) {
        ...
        const sprintMap = new Map<string, { id: string; name: string; state: string }>()
...
export async function getIssueTransitions(issueKey: string): Promise<Array<{ id: string; name: string }>> {
  try {
...
export async function updateIssueStatus(issueKey: string, transitionId: string): Promise<boolean> {
  try {
...
export async function updateIssueAssignee(issueKey: string, accountId: string | null): Promise<boolean> {
  try {
...
export async function getIssues(projectKey: string, filters?: FilterOptions): Promise<JiraIssue[]> {
  try {
    ...
    do {
      ...
      console.log(`Fetched ${data.issues.length} issues. Total so far: ${allIssues.length}/${total}`)

      // Break if we got fewer results than requested (last page)
      if (data.issues.length < maxResults) {
        console.log("Received fewer issues than requested, this was the last page")
        break
      }

      // Safety check to prevent infinite loops
      if (allIssues.length > 10000) {
        console.warn("Reached safety limit of 10,000 issues. Stopping pagination.")
        break
      }
    } while (allIssues.length < total && startAt < total)

    console.log(`Completed fetching ALL ${allIssues.length} issues for selected sprint(s)`)
