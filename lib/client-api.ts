...
export async function fetchProjectUsers(projectKey: string): Promise<JiraUser[]> {
  try {
...
export async function fetchIssueTransitions(issueKey: string): Promise<Array<{ id: string; name: string }>> {
  try {
...
export async function updateIssueAssignee(issueKey: string, accountId: string | null): Promise<boolean> {
  try {
    ...
    if (!response.ok) {
      const errorText = await response.text()
      console.error(`Assignee update failed: ${response.status} ${response.statusText}`, errorText)
      return false
    }
...
export async function fetchIssues(projectKey: string, filters?: FilterOptions): Promise<JiraIssue[]> {
  try {
    ...
    if (filters?.assignee?.length) {
      ...
      const assigneeFilter = filters.assignee.map((assignee) => (assignee === "Unassigned" ? "UNASSIGNED" : assignee))
